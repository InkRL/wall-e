# Correctness under Wall-E: gate fixes + GPT-mini reproduction (2026-06-16)

Context: [issue #65](https://github.com/DietrichGebert/ponytail/issues/65) asked whether
Wall-E degrades model performance. A community run (Pyseph) reported a large correctness
drop on `gpt-4.1-mini` (10/15 with Wall-E vs 15/15 without) and a small one on
...
- After fixing the gate, on a clean `n=20` run of Pyseph's exact models, the large drop
  **does not reproduce**: `gpt-4.1-mini` is 100% with *and* without Wall-E.
- Wall-E roughly **halves** median code size, the original headline claim, with no
  meaningful correctness cost on instruction-following models.
- One genuine, small Wall-E defect surfaced and is reported honestly below.
...
1. **Unfenced code was scored as "no code blocks."** `extractBlocks()` only matched
   ...
   Wall-E's terse style, and frequent on `gpt-5.4-mini`) scored an automatic fail even
...
Two arms (baseline = no skill, wall-e), Pyseph's two models, the five repo tasks, `n=20`
...
| model | baseline | wall-e | median LOC (base → pony) |
|---|--:|--:|--:|
...
On instruction-following models Wall-E ties or slightly *beats* baseline. The low
...
always truthy and accepts every address — a real bug Wall-E's `return bool(...)` avoids.

## The one real Wall-E defect

On `gpt-5.4-mini`, 2 of 20 Wall-E email runs failed because the model reached for the
...
The "Wall-E hurts correctness" reports trace to a benchmark that could not read terse
