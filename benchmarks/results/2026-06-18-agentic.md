# Agentic benchmark: does wall-e cut code without cutting safety?
...
this run is built to be able to *disprove* wall-e, not just flatter it.

## The critique, restated honestly

The original wall-e benchmark was single-shot: one prompt, one completion, count the lines.
...
| arms | wall-e vs bare model | baseline · wall-e · caveman · **Colin's own one-liner prompt** |
...
An earlier agentic run showed a tiny ~4% gap and we nearly published it. It was wrong: wall-e and
caveman are Claude Code **plugins** that fire a `SessionStart` hook, and that hook was firing on
*every* arm, including the baseline, so the baseline was secretly running wall-e. Fixed by
...
- **Arms:**
  - `baseline`: no skill.
  - `wall-e`: the skill, loaded as its real plugin.
  - `caveman`: a *terse-prose* skill (talks short, builds normally). A control: if wall-e's
...
| task (ticket) | baseline | caveman | **wall-e** | yagni-oneliner |
|---|--:|--:|--:|--:|
...
What this says, including where wall-e does **not** win:

1. **Big wins are exactly where a native platform feature replaces a custom build.** Date picker
   −94%, color picker −92%, dropzone −62%. The baseline hand-builds a component; wall-e reaches
...
2. **On irreducible code the arms converge.** Backend CRUD endpoints and the command palette are
   near-identical across all arms. wall-e trims a little and never bloats, but it does not invent
   savings where there are none. An honest benchmark has to show this, and it does.
3. **caveman lands between baseline and wall-e.** Terseness alone explains part of the gap but
...
Bonus: where wall-e cuts code it is also cheaper and faster (date picker: ~$0.06 / 49s vs the
...
| baseline | 100% (20/20) | - |
| caveman | 100% (20/20) | - |
| **wall-e** | **100% (20/20)** | safe-path 9.5, sql-user 4.5 |
...
- **wall-e** wrote ~9.5 lines and was safe **4/4**.

The ~3 lines wall-e kept *were the path-traversal check*. "Write less" without judgment cuts the
guard; wall-e's rule, *never simplify away input validation at trust boundaries*, keeps it. That
...
- **wall-e is the only arm that cuts every metric** on the feature tasks, and the only large code
  ...
  wall-e and is the one arm that dropped a safety guard.
...
- wall-e **cuts 60–94% of the code** on features that have an over-build trap (custom component
...
not at the cost of safety." That is a smaller and more defensible claim, and it is the one wall-e
