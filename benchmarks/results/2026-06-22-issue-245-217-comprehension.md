...
Two issues argued wall-e was lazy in the wrong place:
...
The framing matters: the root-cause fix is presented as the *lazier* (smaller) diff, so wall-e's
...
| model | baseline (no skill) | wall-e (with fix) |
|---|--:|--:|
...
A control confirms it is the *operational* wording, not prose: pre-fix wall-e and a plain-prose
...
**baseline and wall-e both reuse the helper (1.0 each)** — the duplication failure does not
...
Pre-fix vs post-fix wall-e across the full 27-task runnable suite (safety + quality + open/vibe),
Haiku, n=3:
...
- **#245: fixed and validated on the capable tiers** (Sonnet 4.6, the model it was reported on, and
  Opus 4.8): baseline 1/6 → wall-e 6/6, with verified root-cause fixes. Small models remain a
...
`python run.py --task trace-transfer --arms baseline,wall-e --models sonnet --runs 6`.
