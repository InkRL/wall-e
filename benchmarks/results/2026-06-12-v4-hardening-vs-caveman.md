# Wall-E v4 hardening — A–F benchmark vs Caveman (2026-06-12)

Response to the hardening brief in `C:\dev\wall-e-bench\WALL-E-BENCHMARK-WRITEUP.md`.
Harness reused as-is: same 6 tasks (specs reconstructed in `wall-e-bench\specs.md` —
...
verbatim (full level), saved at `wall-e-bench\caveman-SKILL.md`. One fresh
...
numbers are directional; the wall-e4-vs-caveman head-to-head is same-model.
...
2. **Ceiling comments** (5.2): a `wall-e:` shortcut with a known ceiling must
...
guard line in wall-e-review (never flag the minimal check as bloat).

## Build phase — non-blank LOC / .py files (scorer-verified)

| Task | Control (orig) | Treatment v3 (orig) | **Wall-E v4** | **Caveman** |
|---|--:|--:|--:|--:|
...
4. Ceiling-bearing `wall-e:` comments name upgrade paths — **pass**, verified
...
5. Head-to-head vs Caveman — **pass**: ≥ on every axis, strictly better on three.
   ...
   - Reviewability: strictly better — every v4 simplification is `wall-e:`-marked
...
| Whole benchmark (6 builds + C/D extensions) | Control2 | Caveman | Wall-E v4 |
|---|--:|--:|--:|
...
figures (still recorded in `2026-06-12-caveman-vs-wall-e.md`).
...
- Caveman is a prose-compression skill that explicitly writes code "normal" —
  ...
  test reflex did not erode wall-e's size advantage or its 100% probe record.
