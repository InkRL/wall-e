...
> measurement artifact: the wall-e plugin's `SessionStart` hook fired on *every* arm, so the
> "baseline" was secretly running wall-e, which collapsed the gap. With arms properly isolated
> (`--setting-sources project,local` + per-arm `--plugin-dir`) and a real-repo LOC tier added,
> wall-e cuts 60-94% on features with an over-build trap. The safety finding here (the bare
...
- With a **fair baseline** (the real coding agent, not a bare model dumping prose), wall-e's
...
- The interesting result is on the axis the old bench could not see. Two arms dropped safety:
  ...
  (94.4% safe). wall-e, baseline, and caveman stayed **100% safe**.
...
| task | baseline | wall-e | yagni-oneliner |
|---|--:|--:|--:|
| safe-path | 8 | 8 | 7 |
| rate-limit | 18 | 18 | 11 |
| sql-user | 6 | 6 | 4 |
...
baseline and wall-e are essentially tied. wall-e trims a little overall (13.9 vs 14.5 mean)
...
```python
# wall-e: 8 LOC, handles the malformed row
def sum_amount(path):
    total = 0.0
    with open(path, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            try:
                ...
                pass  # wall-e: skip malformed rows, caller gets best-effort sum
    return total
```
...
This is the direct answer to "seven words beat wall-e." On the axis the seven-word benchmark
could not measure, the seven words are the least safe option on the board, and the size they save
over wall-e is about two lines.
...
- It **does** show that a pure "minimize lines" instruction measurably sheds safety, and that
  wall-e keeps the floor at nearly the same size. wall-e was 100% safe and 100% correct
