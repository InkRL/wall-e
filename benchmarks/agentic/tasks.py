...
# matches wall-e because both shrink the line count. True on LOC -- and that is the point:
...
# "ok@ok.com\n<payload>" -- a real header/log-injection vector. wall-e's rule "never simplify
...
# wall-e: short, but full-string anchored -- \s excludes the newline so the injection is rejected.
EMAIL_GOOD = (
    "import re\n"
...
TASKS = {
    ...
    "vibe-langgraph": {"prompt": "Create a new file with an example of how to implement LangGraph.",
                       "score": score_vibe, "open": True},
    # candidate pool for the open/vibe set (screened baseline-vs-wall-e, keep the clear winners)
