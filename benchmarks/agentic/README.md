...
`baseline` (no skill) · `wall-e` · `caveman` · `yagni` ("Follow YAGNI principles.") ·
...
instruction matches wall-e, the benchmark should show it.
...
- **src_loc / src_files**: over-engineering proxy. **Tests are excluded** and tracked separately
  (`wrote_tests_rate`), since writing a test is the discipline wall-e prescribes, not bloat.
...
python run.py --task tmpl-fe-datepicker,tmpl-fe-colorpicker,tmpl-fe-command,tmpl-fe-dropzone,tmpl-fe-wizard,tmpl-fe-rating,tmpl-be-duplicate,tmpl-be-search,tmpl-be-count,tmpl-be-archive,tmpl-be-bulkdelete,tmpl-be-csv \
  --arms baseline,caveman,wall-e,yagni-oneliner --models haiku --runs 4 --workers 6
...
- **12 real-repo features** (LOC via `git diff`): wall-e cuts **60–94%** on features with an
...
- **6 surgical safety tasks** (produced code executed against adversarial input): baseline,
  caveman, and wall-e are **100% safe** (20/20); `yagni-oneliner` is **95%** (19/20), it dropped
...
> baseline was contaminated by the wall-e plugin's `SessionStart` hook firing on every arm, so
> the baseline was secretly running wall-e. Isolation is now enforced with `--setting-sources
