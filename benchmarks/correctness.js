...
function extractBlocks(text) {
  ...
  // wall-e: terse models often answer with bare, unfenced code. Treat the whole
...
// wall-e: probe once at load; macOS and many Linux images ship python3 only.
let pythonCmd;
function python() {
...
function tmpFile(ext, content) {
  const p = path.join(os.tmpdir(), `wall-e-bench-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`);
