// Email under wall-e on Claude (wall-e's primary target), baseline vs wall-e.
const fs = require('fs'), path = require('path');
const { checkPy, pyBlock, TASKS } = require('./robustness-audit.js');
const skill = fs.readFileSync(path.join(__dirname, '..', 'skills', 'wall-e', 'SKILL.md'), 'utf8');
...
(async () => {
  ...
  console.log('model                      baseline   wall-e');
  for (const model of MODELS) {
    const rates = {};
    for (const [arm, sys] of [['baseline', null], ['wall-e', skill]]) {
      let pass = 0, err = 0;
      for (let i = 0; i < N; i++) {
    ...
    console.log(`${model.padEnd(26)} ${rates.baseline.padEnd(10)} ${rates.wall-e}`);
  }
})();
