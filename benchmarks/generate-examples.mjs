...
// each file shows the same task answered with no skill vs with wall-e, same model.
...
Verbatim model output from a benchmark run — Claude Haiku 4.5, no-skill arm vs wall-e arm, temperature 1, source \`benchmarks/output.json\`. Reproduce: \`npx promptfoo@latest eval -c benchmarks/promptfooconfig.yaml\`.

## Without Wall-E — ${bL} lines of code

${b.response.output.trim()}

## With Wall-E — ${pL} lines of code
...
with no skill (\`## Without Wall-E\`) and with wall-e (\`## With Wall-E\`), so you can
