...
// always-on context, commands/*.toml for /wall-e + /wall-e-review, and
...
// carries the load-bearing rules — i.e. if the adapter stops wiring wall-e.
...
const REUSED_COMMANDS = ['commands/wall-e.toml', 'commands/wall-e-review.toml'];
const REUSED_SKILLS = ['skills/wall-e/SKILL.md'];
// Gemini CLI auto-loads this exact path for extension hooks. Wall-E's
...
test('manifest names the wall-e extension with a pinned version', () => {
...
test('contextFileName resolves to a file carrying the wall-e rules', () => {
  const manifest = loadManifest();
