#!/usr/bin/env node
// Every wall-e command the pi extension registers must also ship as a
// file-based command for the hosts that need one: Claude Code (commands/*.toml,
// which Gemini CLI reuses) and OpenCode (.opencode/command/*.md). /wall-e-help
...
test('pi registers at least the base command', () => {
  assert.ok(commands.includes('wall-e'), 'expected pi to register a wall-e command');
});
